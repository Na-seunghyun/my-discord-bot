...
voice_channel_join_times = {}  # 채널 ID: 입장 시간
play_queue = Queue()
currently_processing_channels = set()
...
async def monitor_voice_channels():
    ...
    for guild in bot.guilds:
        for channel in guild.voice_channels:
            if channel.name not in MONITORED_CHANNEL_NAMES:
                continue

            members = [m for m in channel.members if not m.bot]
            if members:
                if channel.id not in voice_channel_join_times:
                    voice_channel_join_times[channel.id] = now
                else:
                    ...
                    if (
                        ...
                        await play_queue.put(channel)
                        currently_processing_channels.add(channel.id)
            else:
                # 사람이 없으면 타이머 초기화
                voice_channel_join_times.pop(channel.id, None)

    # 현재 대기 중인 채널 로그 출력
    if not play_queue.empty():
        pending_channels = list(play_queue._queue)
        pending_names = [ch.name for ch in pending_channels]
        print(f"🕓 대기 중인 채널들: {', '.join(pending_names)}")

@tasks.loop(seconds=5)
async def process_play_queue():
    while not play_queue.empty():
        channel = await play_queue.get()
        try:
            await play_audio_and_leave(channel)
        finally:
            currently_processing_channels.discard(channel.id)
            play_queue.task_done()

async def play_audio_and_leave(channel: discord.VoiceChannel):
    if channel.guild.voice_client:
        return  # 이미 봇이 연결 중

    temp_file = f"temp_{channel.id}.mp3"

    try:
        vc = await channel.connect()
        audio_url = (
            "https://raw.githubusercontent.com/Na-seunghyun/my-discord-bot/main/"
            "%EC%9D%8C%EC%84%B1%EC%B1%84%EB%84%902%EC%8B%9C%EA%B0%84%EA%B2%BD%EA%B3%BC.mp3"
        )

        async with aiohttp.ClientSession() as session:
            async with session.get(audio_url) as resp:
                if resp.status != 200:
                    print("⚠️ 오디오 파일 다운로드 실패")
                    await vc.disconnect()
                    return
        ...
        vc.play(discord.FFmpegPCMAudio(temp_file), after=lambda e: print("🎵 재생 종료"))
        ...
        try:
            if os.path.exists(temp_file):
                os.remove(temp_file)
                print(f"🧹 '{channel.name}'의 임시 파일 삭제 완료")
        except Exception as cleanup_err:
            print(f"⚠️ 파일 삭제 오류: {cleanup_err}")
