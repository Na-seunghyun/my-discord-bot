...
active_gift_games = {}  # game_id → {"players": [], "bet": int}
...
@tree.command(name="선물고르기", description="🎁 1명에게 몰빵, 자동 선택 포함!", guild=discord.Object(id=GUILD_ID))
@app_commands.describe(베팅액="1인당 베팅액 (최소 1,000원)")
async def 선물고르기(interaction: discord.Interaction, 베팅액: int):
    if 베팅액 < 1000:
        return await interaction.response.send_message("❌ 최소 베팅 금액은 1,000원입니다.", ephemeral=True)

    game_id = str(interaction.id)
    active_gift_games[game_id] = {"players": [], "bet": 베팅액}

    class GiftJoinView(discord.ui.View):
        def __init__(self):
            super().__init__(timeout=10)

        @discord.ui.button(label="🎁 참여하기", style=discord.ButtonStyle.primary)
        async def join_button(self, btn_int: discord.Interaction, button: discord.ui.Button):
            uid = str(btn_int.user.id)
            game = active_gift_games.get(game_id)
            if not game:
                return await btn_int.response.send_message("❌ 유효하지 않은 게임입니다.", ephemeral=True)

            if uid in game["players"]:
                return await btn_int.response.send_message("⚠️ 이미 참여하셨습니다.", ephemeral=True)

            if get_balance(uid) < 베팅액:
                return await btn_int.response.send_message("💸 잔액 부족입니다.", ephemeral=True)

            game["players"].append(uid)
            add_balance(uid, -베팅액)
            try:
                if not btn_int.response.is_done():
                    await btn_int.response.send_message("✅ 참여 완료!", ephemeral=True)
            except Exception as e:
                print(f"⚠️ 참여 응답 실패: {e}")
    ...
    async def run_game():
        ...
        if len(players) < 2:
            msg = "❌ 최소 2명 이상 필요합니다. 취소합니다."
            try:
                await interaction.followup.send(msg)
            except:
                if not interaction.response.is_done():
                    await interaction.response.send_message(msg)
            del active_gift_games[game_id]
            return

        # 🎬 참여자 선택 연출
        mentions = [get_mention(u) for u in players]
        try:
            await interaction.followup.send(f"🎁 참여자: {', '.join(mentions)}")
        except:
            pass
        ...
        selected = None
        # 안내 메시지
        choice_msg = await interaction.followup.send("지금 골라주세요! (2초)")
        try:
            def check(i: discord.Interaction):
                return (
                    i.message.id == choice_msg.id
                    and str(i.user.id) in players
                    and i.data.get("custom_id") == "🎁 참여하기"
                )

            sel_int = await bot.wait_for("interaction", timeout=2.0, check=check)
            selected = str(sel_int.user.id)
            if not sel_int.response.is_done():
                await sel_int.response.send_message("🎉 직접 선택하셨습니다!", ephemeral=True)
        except asyncio.TimeoutError:
            selected = random.choice(players)
            try:
                await interaction.followup.send("⏱ 시간 초과! 자동으로 선택되었습니다.")
            except:
                pass
        except Exception as e:
            print(f"⚠️ 자동 선택 오류: {e}")
            selected = random.choice(players)

        # 결과 발표
        total = len(players) * bet
        add_balance(selected, total)

        lines = [f"🎉 {get_mention(selected)} 님이 🎁 성공적으로 선물을 골랐습니다!"]
        lines.append(f"💰 상금: **{total:,}원** 몰빵 획득!")
        lines.append("👏 나머지 참가자들도 수고하셨습니다!")

        embed = discord.Embed(title="🎁 선물 고르기 결과", description="\n".join(lines), color=discord.Color.gold())
        try:
            await interaction.followup.send(embed=embed)
        except:
            if not interaction.response.is_done():
