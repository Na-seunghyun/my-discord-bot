...
def update_leaderboard_collection(nickname: str):
    try:
...
async def start_pubg_collection():
    ...
    while True:
        # ✅ [1] 즉시 실행
        print("🚀 [즉시 수집] 봇 시작 시 1회 자동 수집 실행 중...")
        await run_pubg_collection(manual=True)
        ...
        print(f"🕓 [예약 대기] 다음 수집까지 대기: {target.strftime('%Y-%m-%d %H:%M')} ({wait_seconds/60:.1f}분)")
        ...
        for i, member in enumerate(members, start=1):
            ...
            try:
                if not can_make_request():
                    print(f"⏳ API 제한 → 대기 후 재시도: {nickname}")
                    await asyncio.sleep(60)
                    continue
        ...
        try:
            ...
            if self.label == self.correct_slot:
                ...
                desc = (
                    ...
                    f"\n💰 잔액: **{await get_balance(self.user_id):,}원**"
                    f"{title_str}"
                )
                color = discord.Color.green()
...
async def 복권(interaction: discord.Interaction, 베팅액: int):
    ...
    if await get_balance(user_id) < 베팅액:
        return await interaction.response.send_message(
            embed=create_embed("💸 잔액 부족", f"잔액: **{await get_balance(user_id):,}원**", discord.Color.red()),
            ephemeral=False
        )
...
async def 도박배틀(interaction: discord.Interaction, 대상: discord.Member, 배팅금액: int):
    ...
    class BattleConfirmView(discord.ui.View):
        ...
        async def accept(self, interaction: discord.Interaction, button: discord.ui.Button):
            ...
            # ✅ 잔액 재확인
            if await get_balance(str(self.caller.id)) < self.amount or await get_balance(str(self.target.id)) < self.amount:
                self.stop()
            ...
            # ✅ 현재 잔액
            caller_amount = await get_balance(str(self.caller.id))
            target_amount = await get_balance(str(self.target.id))
...
class RealEstateView(ui.View):
    ...
    def make_callback(self, region: str):
        async def callback(interaction: Interaction):
            ...
            balance = await get_balance(self.user.id)
